...
import { getMarks, deleteMarksById } from 'apiSdk/marks';
import { MarksInterface } from 'interfaces/marks';

type ColumnType = ColumnDef<MarksInterface, unknown>;

interface MarksListPageProps {
...
export function MarksListPage(props: MarksListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<MarksInterface>>(
    () => `/marks?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: MarksInterface) => {
    if (hasAccess('marks', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/marks/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/marks/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('marks', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/marks/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('marks', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="marks" />
        </Flex>

        {hasAccess('marks', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/marks/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
